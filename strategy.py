...
class ScopaStrategy:
    ...
    @staticmethod
    def __get_potential_moves(scopa_board: list[ScopaCard], hand: list[ScopaCard]) -> tuple[list[ScopaMove], ScopaMoveType]:
        potential_moves = []
        single_take = set()
        move_type = None
    ...
    def __get_move_scores(self, potential_moves: list[ScopaMove], board: list[ScopaCard], move_type: ScopaMoveType) -> list[int]:
        ...
        if move_type == ScopaMoveType.TAKE:
            ...
            for i, pm in enumerate(potential_moves):
                ...
                for c in cards_to_take:
                    if c.rank() == ScopaRank.SEVEN and c.suit() == ScopaSuit.COINS:
                ...
                coin_cards = [c for c in cards_to_take if c.suit() == ScopaSuit.COINS]
    ...
    def make_move(self, scopa_board: list[ScopaCard], hand: list[ScopaCard]) -> ScopaMove:
        ...
        potential_moves, move_type = ScopaStrategy.__get_potential_moves(scopa_board, hand)

        if len(potential_moves) == 1:
            return potential_moves[0]

        scores = self.__get_move_scores(potential_moves, scopa_board, move_type)
